...
export default function Receipts() {
  ...
  const handleValidate = async (id) => {
    if (!confirm('Are you sure you want to validate this receipt? Stock will be updated.')) {
      return;
    }
    try {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={handleNewReceipt} className="btn btn-primary flex items-center gap-2">
                ...
                receipts.map((receipt) => (
                  <tr key={receipt.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-3 px-4">
                      <span className="text-sm font-medium">{receipt.documentNumber}</span>
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4">
                      <span className="text-sm">{receipt.warehouse?.code || '-'}</span>
                    </td>
                    <td className="py-3 px-4">
                      <span className="text-sm">{receipt.items?.length || 0} items</span>
                    </td>
                    <td className="py-3 px-4">
                      <span className={`text-sm font-medium ${
                        receipt.status === 'VALIDATED' ? 'text-success' : 'text-warning'
                      }`}>
                        {receipt.status}
                      </span>
                    </td>
