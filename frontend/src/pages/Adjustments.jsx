...
export default function Adjustments() {
  ...
  const addItem = () => {
    setFormData({
      ...formData,
      items: [...formData.items, { productId: '', locationId: '', physicalCount: 0 }]
    });
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={handleNewAdjustment} className="btn btn-primary flex items-center gap-2">
                ...
                adjustments.map((adjustment) => (
                  <tr key={adjustment.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-3 px-4">
                      <span className="text-sm font-medium">{adjustment.documentNumber}</span>
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4">
                      <span className="text-sm">{adjustment.warehouse?.code || '-'}</span>
                    </td>
                    <td className="py-3 px-4">
                      <span className="text-sm">{adjustment.items?.length || 0} items</span>
                    </td>
                    <td className="py-3 px-4">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-2xl font-semibold">New Adjustment</h2>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  className="input"
                  placeholder="e.g., Stocktake, Damage, Theft"
                  required
                />
              </div>
              <div className="mb-4">
                  ...
                  <div className="space-y-2">
                    {formData.items.map((item, index) => (
                      <div key={index} className="grid grid-cols-12 gap-2 items-end p-3 border border-gray-200 rounded">
                        ...
                        <div className="col-span-4">
                          <label className="block text-xs text-gray-600 mb-1">Physical Count *</label>
                          <input
                            type="number"
                            value={item.physicalCount}
                            onChange={(e) => updateItem(index, 'physicalCount', parseInt(e.target.value) || 0)}
                            className="input text-sm"
                            min="0"
                            required
